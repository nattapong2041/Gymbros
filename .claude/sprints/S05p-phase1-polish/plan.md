...
| Next step | Re-run manual smoke for local notification fire/tap-back, global keyboard dismissal, last-session row copy, responsive table column alignment on iPhone/iPad, and rest timer Ready state in app/Live Activity/Dynamic Island. |
...
- [ ] Re-test after follow-up: verify last-session row for same-program history, fallback history, and no-history states, with no repeated weight unit in the row text.
