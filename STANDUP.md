...
- Implemented Sprint 6.1 — Post-Launch Feature Wave (4 of 5 features), via subagent-driven-development directly on `main`, 18 tasks, commits `07ceda7`..`3d204cc`:
  ...
  - **Final whole-branch review caught a real functional bug the per-task reviews couldn't see**: `TodayViewModel.fetchBudgetedSets` only fetched sets for the 4 most *recent* sessions on normal days, but `StallDetector` needs 4 *qualifying* (same-exercise) sessions — on any multi-day program (Upper/Lower, PPL, the norm for real users), the 4 most recent sessions rarely all share one exercise, so the Overload Advisor could essentially never fire. Fixed by reusing the wider 14-session comeback-baseline budget on every day (`fetchBudgetedSets`, commit `f095b0b`), plus a new regression test (`stalledExerciseDetectedAcrossInterleavedMultiDayRotation`) proving detection across an interleaved 2-day rotation. Re-verified: full build + full `GymbrosTests` suite green after the fix.
...
- [ ] Sprint 6.1 final-review Minor findings, deferred (none block shipping): (a) `OverloadAdvisorCardView` shows the recommended day's stalled exercise even after "Change day" swaps to a different day — data isn't wrong, but placement can read as belonging to the wrong day; (b) `today.overload_advisor.body` hardcodes "4 sessions" rather than interpolating `StallDetector.sessionThreshold`, and uses non-positional `%@` (fine today since en/th share word order, latent hazard for future translations); (c) the 14-session fetch window can still under-fire on 4+ day program rotations, and the 14 `fetchSets` calls in `fetchBudgetedSets` run sequentially rather than concurrently — worth revisiting if Today-load latency or advisor recall on longer splits becomes a real issue.
